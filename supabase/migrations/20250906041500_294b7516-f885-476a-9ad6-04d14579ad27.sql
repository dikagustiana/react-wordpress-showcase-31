-- Fix trigger conflict and create tables for FSLI inline editing system

-- Drop existing trigger if it exists
DROP TRIGGER IF EXISTS update_fsli_sections_updated_at ON public.fsli_sections;
DROP TRIGGER IF EXISTS update_fsli_embeds_updated_at ON public.fsli_embeds;

-- Create tables for FSLI inline editing system (using different names to avoid conflicts)
...
CREATE TRIGGER update_fsli_inline_sections_updated_at
BEFORE UPDATE ON public.fsli_inline_sections
FOR EACH ROW
EXECUTE FUNCTION update_updated_at_column();

CREATE TRIGGER update_fsli_inline_embeds_updated_at
BEFORE UPDATE ON public.fsli_inline_embeds
FOR EACH ROW
EXECUTE FUNCTION update_updated_at_column();