...
-- First, let's check if we have proper admin role checking
-- Update the RLS policy to allow admins to insert essays
...
VALUES 
  ('assets', 'assets', true),
  ('content-green-transition', 'content-green-transition', true)
...
-- Create storage policies for content-green-transition bucket
CREATE POLICY "Anyone can view green transition content" 
ON storage.objects 
FOR SELECT 
USING (bucket_id = 'content-green-transition');

CREATE POLICY "Only admins can upload green transition content" 
ON storage.objects 
FOR INSERT 
WITH CHECK (
  bucket_id = 'content-green-transition' AND 
  auth.uid() IS NOT NULL AND (
    (auth.jwt() -> 'app_metadata' ->> 'role') = 'admin' OR
    EXISTS (SELECT 1 FROM public.user_roles ur WHERE ur.user_id = auth.uid() AND ur.role = 'admin')
  )
